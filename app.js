...
async function fetchEvents() {
    ...
    console.log("Type \t \t Sender \t Recipient \t Amount USDC \t Amount ETH \t Price \t \t \t Price in USDC");
    console.log("-----------------------------------------------------------------------------------------------------------------");
    ...
    contract.on("Swap", (sender, recipient, amount0, amount1, sqrtPriceX96, liquidity, tick) => {
        ...
        const Q96 = 2 ** 96;
        let ethPriceInUSDC = (Number(sqrtPriceX96) / Q96) ** 2;

        const formatAddress = (addr) => `${addr.slice(0, 6)}...${addr.slice(-4)}`;
        
        if (amount0 > 0) {
            console.log("Buy ETH \t", formatAddress(sender) ,"\t", formatAddress(recipient), "\t", usdcAmount, "\t", -(ethAmount), "\t$", -(price), "\t$", ethPriceInUSDC);
        } else {
            console.log("Sell ETH\t", formatAddress(sender), "\t", formatAddress(recipient), "\t", -(usdcAmount), "\t", ethAmount ,"\t$", -(price), "\t$", ethPriceInUSDC);
        }
    });
}
